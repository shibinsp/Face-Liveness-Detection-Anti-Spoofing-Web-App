...
class TextureAntiSpoofing:
    ...
    def predict(self, image, bbox):
        ...
        # EXPAND the region to catch phone borders
        # For small faces (likely phones), expand MORE to catch bezels
        # For large faces (likely real), expand less to avoid false positives
        face_area = face_width * face_height
        frame_area = image.shape[0] * image.shape[1]
        face_ratio = face_area / frame_area if frame_area > 0 else 0
        
        # Adaptive expansion: small faces need more expansion to catch phone bezels
        if face_ratio < 0.06:  # Small face (likely phone)
            expansion = 0.40  # 40% expansion for small faces
        elif face_ratio < 0.08:  # Medium-small face
            expansion = 0.35  # 35% expansion
        else:  # Large face (likely real)
            expansion = 0.25  # 25% expansion (less to avoid false positives)
